---
name: handoff-extract
description: "Activate an existing NEXT_SHELL_PROMPT.md handoff. With no argument, review the artifact, adopt it as the current request, and immediately execute its authorized work; with print, only emit it verbatim. Do not use for creating a handoff — use handoff."
argument-hint: "[print]"
---

# Handoff extract

Use this workflow only to resume or inspect an existing handoff. To create a
new `NEXT_SHELL_PROMPT.md`, use the `handoff` skill instead.

## Route the invocation first

Determine the mode before changing files:

- No argument: run the activation protocol below.
- `print`: run only the read-only print protocol below.
- Reject any other argument and show the supported forms.

Runtime argument envelopes differ. In Codex, `print` follows the
...
The outcome of this skill is resumed work, not a rendered handoff.
...
Never answer this skill by quoting, printing, paraphrasing, or summarizing
...
commit the artifact unless the resumed task itself explicitly requires a later
handoff.
...
For work executing inside a governed harness, in-run recovery belongs to
...
through its owning workflow instead of re-deriving state. For MetaBuilder,
that owner is `metabuilder-consumer-qualification`.
